...
saveFileName = 'use myNet & train in subset2 & VAT'
class trainer():
    ...
    def train(self,device):
        ...
        Dict = torch.load('/home/stu013/mapping/HBDW_v3/modelParam/MESSIDOR/use myNet & train in subset2/modelParam_50_6.436375760195548.pth')
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
        # Scheduler 学习率下降曲线
        lf = lambda x: ((1 + math.cos(x * math.pi / 20)) / 2) * (1 - lrf) + lrf  # cosine
        ...
        # 获取模板图片
        templatePath1 = './template_sub2_strid_20.jpg'
        ...
        for epoch in range(num_epoch):
            ...
            for i, data in tqdm(enumerate(train_LD)):
                ...
                # print(data[1][0])
                #batch_loss = loss(main_Pred.cuda(device), data[1].cuda(device)) # 计算 loss （注意 prediction 跟 label 必须同时在 CPU 或是 GPU 上）
                if epoch > 3:
                    lds = vat_loss(model, data[0].cuda(device), data[0].clone().cuda(device),template1.cuda(device))

                main_Pred = model(data[0].cuda(device), data[0].clone().cuda(device),template1.cuda(device))  # 利用 model 得到预测的概率分布，这边实际上是调用模型的 forward 函数
            ...
            with torch.no_grad():
                ...
                for i, data in tqdm(enumerate(val_LD)):
                    val_pred = model(data[0].cuda(device), data[0].clone().cuda(device),template1.cuda(device))
                ...
                if epoch == 0:
                    f = open('./outputFile/MESSIDOR/{}/detail.txt'.format(saveFileName),'a+')
                    f.write('template is {}\n\r'.format(templatePath1))
