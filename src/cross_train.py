...
from network_3.mainNet.mainNet import mainNet
    ...
    def train(self):
        ...
        for epoch in range(num_epoch):
            ...
            for i, data in enumerate(train_LD):
                ...
                main_Pred,fm1Pred,fm2Pred,splitPoint = model(data[0].cuda(device), data[0].detach().cuda(device),epoch,i,'train')  # 利用 model 得到预测的概率分布，这边实际上是调用模型的 forward 函数
                split_label = data[1].detach() - (splitPoint-64)
                batch_loss = a1*loss(main_Pred.cuda(device), data[1].cuda(device))+a2*loss(fm1Pred.cuda(device), split_label.cuda(device))\
                             +a3*loss(fm2Pred.cuda(device), split_label.cuda(device))  # 计算 loss （注意 prediction 跟 label 必须同时在 CPU 或是 GPU 上）
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(val_LD):
                    main_Pred,fm1Pred,fm2Pred,splitPoint = model(data[0].cuda(device), data[0].detach().cuda(device),epoch,i,'val')
                    split_label = data[1].detach() - (splitPoint - 64)
                    # batch_loss = sum([(x - y) ** 2 for x, y in zip(data[1].cuda(), val_pred)]) / len(train_pred)
                    batch_loss = a1*loss(main_Pred.cuda(device), data[1].cuda(device)) + a2*loss(fm1Pred.cuda(device),
                                                                                           split_label.cuda(device)) \
                                 + a3 * loss(fm2Pred.cuda(device), split_label.cuda(device))
                    # val_acc += np.sum(np.argmax(val_pred.cpu().data.numpy(), axis=1) == data[1].numpy())
                    fm1Pred = (splitPoint.cuda(device)-64)+fm1Pred
                    fm2Pred = (splitPoint.cuda(device)-64)+fm2Pred
                    val_pred = a1*main_Pred+a2*fm1Pred+a3*fm2Pred
                ...
                if os.path.exists('outputFile/MESSIDOR/TRv2') is False:
                    os.makedirs('outputFile/MESSIDOR/TRv2')
                dataframe.to_csv('./outputFile/MESSIDOR/TRv2/val_detail_{}.csv'.format(epoch), sep=',')
                # 将结果 print 出來
                ...
                if epoch >= 80 :
                    if dist < best_dist:
                        ...
                        torch.save(best_model,'./modelParam/MESSIDOR/6/modelParam_{}_{}.pth'.format(epoch,dist))
