...
saveFileName = 'train in subset2 & datasplit T_V_T & val_num 20% & train in network & using crossVal & not dataAug & bs 8 & split strid 1 & label not int & newLossFunction & realR8rate & lrsch 20'
class trainer():
    ...
    def train(self,device):
        ...
        for epoch in range(num_epoch):
            ...
            for i, data in tqdm(enumerate(train_LD)):
                ...
                # print(data[1][0])
                batch_loss = self.lossFunction(main_Pred.cuda(device), data[1].cuda(device)) # 计算 loss （注意 prediction 跟 label 必须同时在 CPU 或是 GPU 上）
