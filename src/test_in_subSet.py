import numpy as np
import torch
import dataprocess.crossval_tvt_subset2 as DL
...
saveFileName = 'train in subset2 & datasplit T_V_T & val_num 20_25 & train in network & using crossVal & not dataAug & bs 8 & split strid 1 & label not int & newLossFunction & realR8rate & lrsch 20'
epoch_num = 472
class trainer():
    ...
    def train(self,device):
        ...
        Dict = torch.load(
            '/home/stu013/mapping/HBDW_v3/modelParam/MESSIDOR/train in subset2 & datasplit T_V_T & val_num 20_25 & train in network & using crossVal & not dataAug & bs 8 & split strid 1 & label not int & newLossFunction & realR8rate & lrsch 20/modelParam_472_5.75819140149836.pth',
            map_location={'cuda:0': 'cuda:1'})

        # Dict = torch.load('/home/stu013/mapping/HBDW_v3/modelParam/MESSIDOR/crossval valset 10-2 &train in network & using crossVal & VAT & bs 4 & split strid 1 & label not int & newLossFunction & realR8rate/modelParam_265_3.208080624279223.pth',map_location={'cuda:0':'cuda:1'})
        ...
        batchSize = 1
        _, _ , test_loader,_,test_imgNames = DL.getDataLoader(batchSize)
        # np.savetxt('vn.txt',val_image_names)


        logging.info('batchsize is {}'.format(batchSize))

        computer = computerRrate_test(1)
        ...
        with torch.no_grad():
            ...
            count = computer.computer(pred_site, os.path.join('testInMessidor/test_in_subset1', saveFileName), epoch_num, test_imgNames.reshape(-1))
            ...
            if os.path.exists('./testInMessidor/test_in_subset1/{}'.format(saveFileName)) is False:
                os.makedirs('./testInMessidor/test_in_subset1/{}'.format(saveFileName))
            dataframe.to_csv('./testInMessidor/test_in_subset1/{}/test_detail_{}.csv'.format(saveFileName,epoch_num), sep=',')
