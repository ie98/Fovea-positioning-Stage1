...
class MyDataSet(Dataset):
    ...
    def __init__(self, x_img_name, y_label, transforms=None):
        ...
        self.label_matrix = np.zeros(shape=[3, 512, 512], dtype=np.uint8)
        #
        # # 外围
        # self.label_matrix[0, 0:100, :] = 1
        # self.label_matrix[0, -100:, :] = 1
        # self.label_matrix[0, :, 0:100] = 1
        # self.label_matrix[0, :, -100:] = 1
        #
        # # 中间
        # self.label_matrix[1, 77:177, 77:433] = 1
        # self.label_matrix[1, -177:-77, 77:433] = 1
        # self.label_matrix[1, 77:433, 77:177] = 1
        # self.label_matrix[1, 77:433, -177:-77] = 1
        #
        # # 内围
        # self.label_matrix[2, 155:355, 155:355] = 1

        # 外围
        self.label_matrix[0, 0:150, :] = 1
        self.label_matrix[0, -150:, :] = 1
        self.label_matrix[0, :, 0:150] = 1
        self.label_matrix[0, :, -150:] = 1

        # 中间
        self.label_matrix[1, 110:231, 110:400] = 1
        self.label_matrix[1, -231:-110, 110:400] = 1
        self.label_matrix[1, 110:400, 110:231] = 1
        self.label_matrix[1, 110:400, -231:-110] = 1

        # 内围
        self.label_matrix[2, 205:305, 205:305] = 1
    ...
    def __getitem__(self, item):
        ...
        l_a,l_b,l_c = self.labelChange(Y_label)
        return X_img, l_a,l_b,l_c,Y_label
    ...
    def labelChange(self,label):
        label_a = np.zeros(shape=[2],dtype=np.float32)  # c 负责 [0,99] and [411,511]
        label_b = np.zeros(shape=[2],dtype=np.float32)  # b 负责 [77,177] and [333,433]
        label_c = np.zeros(shape=[2],dtype=np.float32) # a 负责 [155,355]


        col = int(label[0])
        row = int(label[1])

        if self.label_matrix[0,row,col] == 1:
            label_a = label.astype(np.float32)

        if self.label_matrix[1,row,col] == 1:
            label_b = label.astype(np.float32)

        if self.label_matrix[2,row,col] == 1:
            label_c = label.astype(np.float32)
...
def getDataLoader():
    ...
    return train_loader,val_loader,val_imgName
