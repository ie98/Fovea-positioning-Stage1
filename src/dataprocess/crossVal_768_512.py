...
def loadImgs(img_name,label):
    ...
    # 图像下采样
    img.astype(np.uint8)
    img = cv2.resize(img, dsize=(downSample_W,downSample_H), interpolation=cv2.INTER_LINEAR)
...
def getDataLoader(batchSize):
    ...
    #np_list = np.array(random_list)
    train_imgNames = imgNames[sub_set_1]
    train_labels = labels[sub_set_1].astype(np.float32)
    #train_imgNames = train_imgNames[np_list]
    #train_labels = train_labels[np_list]
    val_imgName = imgNames[sub_set_2]
    val_labels = labels[sub_set_2].astype(np.float32)
