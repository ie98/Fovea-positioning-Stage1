...
class MyDataSet(Dataset):
    ...
    def __init__(self, x_img_name, y_label, transforms=None,fun=None):
    ...
    def dataAug(self,x):
        ...
        if p >= 0.6:
            ...
            # print('q = {}'.format(q))
            noise = np.random.normal(0, q, size=(x.size(1), x.size(2))).astype(np.float32)
...
def getDataLoader():
    ...
    train = 0
    val = 1
    train_set = MyDataSet(train_imgNames,train_labels,input_transform,train)
    val_set = MyDataSet(val_imgName,val_labels,input_transform,val)
